...
| `paused_at` | `timestampTz` | yes | — | when voucher was paused | **lifecycle timestamp** |
| `depleted_at` | `timestampTz` | yes | — | when usage limit was reached | **lifecycle timestamp** |
| `last_activated_at` | `timestampTz` | yes | — | when voucher was (re)activated | **lifecycle timestamp** |

**State machine transitions** (from `VoucherStatus::config()`):
```
Active ──→ Paused    → SET paused_at = NOW()
Active ──→ Expired   → (handled by expires_at)
Active ──→ Depleted  → SET depleted_at = NOW()
Paused ──→ Active    → SET last_activated_at = NOW(), SET paused_at = NULL
Paused ──→ Expired   → (handled by expires_at)
...
| `created_at` | timestampTz | no | auto | record creation | — |
| `updated_at` | timestampTz | no | auto | last write | — |
| `status` | `string(50)` | no | `'active'` | active, expired, revoked | **lifecycle state** |
| `revoked_at` | `timestampTz` | yes | — | when assignment was revoked | **lifecycle timestamp** |

**Status**: `status` and `revoked_at` columns added. `VoucherAssignment` Eloquent model created.
